...
void FilamentApp::run(const Config& config, SetupCallback setupCallback,
    ...
    loadIBL(config);
    loadSkybox(config);
    if (mIBL != nullptr && mSkybox != nullptr) {
        // Shapr: a separately loaded skybox overrides the one that comes with the IBL
...
void FilamentApp::Window::configureCamerasForWindow() {
    ...
    mDebugCamera->setProjection(45.0, double(mainWidth) / height, 0.0078125, 4096, Camera::Fov::VERTICAL);
