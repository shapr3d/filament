...
// When CONFIG_MAX_INSTANCES > 1, SPIR-V inside ANGLE generates invalid HLSL.
// In the generated HLSL vertex shaders, per-renderable data is not taken as a cbuffer
// but as a StructuredBuffer and it causes alignment issues during runtime
constexpr size_t CONFIG_MAX_INSTANCES = 1;
#else
