...
// Matches logic in filament/backend/src/PlatformFactory.cpp for Backend::DEFAULT
        ...
        "   --skybox=<path>, -Y <path>\n"
        "       Override the built-in skybox and the one from the IBL.\n"
        "       Path can either be a directory containing IBL data files generated by cmgen,\n"
        "       or, a .hdr equiretangular image file\n\n"
        "   --actual-size, -s\n"
...
static int handleCommandLineArguments(int argc, char* argv[], App* app) {
    static constexpr const char* OPTSTR = "ha:f:i:Y:usc:y:rt:b:evg:";
    ...
    while ((opt = getopt_long(argc, argv, OPTSTR, OPTIONS, &option_index)) >= 0) {
        ...
        switch (opt) {
            ...
            case 'i':
                ...
                std::cout << "detecting ibl dir as: " << arg << "\n";
                break;
            case 'Y':
                app->config.skyboxDirectory = arg;
                std::cout << "detecting skybox dir as: " << arg << "\n";
                break;
            case 'u':
        ...
        app.viewer->setUiCallback([&app, scene, view, engine] () {
            ...
            if (ImGui::CollapsingHeader("Automation", flags)) {
                ...
                if (ImGui::Button("Export view settings")) {
                    AutomationEngine::exportSettings(app.viewer->getSettings(), settingsFilePath.c_str());
