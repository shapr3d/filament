...
public:
    ...
    struct RenderPrimitive {
        ...
        // Shapr: index buffers are backed by BufferObjects that can be swapped at runtime
        // (see OpenGLDriver::setIndexBufferObject()), so the VAO needs to know its index buffer.
        Handle<HwIndexBuffer> indexBuffer;                      // 4

        // Shapr: if this differs from indexBuffer->bufferObjectVersion, the element array
        // binding of this VAO needs to be updated (see OpenGLDriver::bindRenderPrimitive())
        uint32_t indexBufferVersion = 0;                        // 4
        ...
        // Shapr: widened to 32 bits to match HwVertexBuffer::bufferObjectsVersion
        uint32_t vertexBufferVersion = 0;                       // 4
