...
id<MTLArgumentEncoder> ArgumentEncoderCreator::operator()(id<MTLDevice> device,
    ...
    if (textureCount > maxArgumentBufferSamplerCount) {
        utils::slog.e << "Error: max sampler per argument buffer supported by this GPU (" << maxArgumentBufferSamplerCount << ") exceeded (" << textureCount << ")" << utils::io::endl;
    }
